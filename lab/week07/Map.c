...
//               list     int                 int        int array
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
   assert(g != NULL);
 	//two seas ? boat connection, return 1;
   if(idToType(start) == SEA && idToType(end) == SEA){
   		type[0] = BOAT;
   		return 1;
   }
	int count = 0;
   VList curr = NULL, curr2 = NULL;
   for(curr = g->connections[start];curr != NULL;curr = curr->next){
   //rail and road connection
	   	if(curr->v == end){
	   		type[count++] = curr->type;
	   	}//port cities? boat connection 
	   	else if(curr->type == BOAT){
	   		for(curr2 = g->connections[curr->v];curr2 != NULL;curr2 = curr2->next){
                if (curr2->v == end && curr2->type == BOAT)
                    type[count++] = curr->type;
            }
		}
   }
