...
{
	assert(L != NULL); 
	struct DLListNode *newNode = newDLListNode(it);  //create a node pointer to store the node
	if(L->curr == NULL){
        L->first = L->last = L->curr = newNode; // empty list
...
{
	assert (L != NULL);
	struct DLListNode *freedNode;
	if(L->nitems == 0) return; //empty list
	//current is last
	else if(L->curr == L->last){
		freedNode = L->last;
		L->last = L->last->prev;
...
void DLListDelete(DLList L)
{
    ...
    struct DLListNode *newcurr = L->curr->next;
    L->curr->prev->next = L->curr->next;
    L->curr->next->prev = L->curr->prev;
    free(L->curr);
    L->curr = newcurr;
